...
// in-place with O(1) extra memory. Clarification: Confused why the returned
// value is an integer, but your answer is an array? Note that the input array
// is passed in by reference, which means a modification to the input array will
// be known to the caller. Internally you can think of this:
...
class Solution {
 public:
  int removeDuplicates(vector<int>& nums) {
    int n = nums.size(), tail = n - 1, prevIdx = 0, idx = 1;
    if (n <= 2) return n;
    ...
    prevIdx = idx;
    while (idx < n) {
      while (idx < n && nums[idx] == INT_MIN) idx++;
      while (idx < n && nums[idx] != INT_MIN) {
        if (prevIdx == idx) {
          prevIdx++;
          idx++;
        } else
