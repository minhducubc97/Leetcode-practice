...
class Solution {
 public:
  vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
    ...
    combinationSumRecursion(candidates, target, result, curSeries, 0);
    return result;
  }

  void combinationSumRecursion(vector<int>& candidates, int target,
                               vector<vector<int>>& result,
                               vector<int> curSeries, int curIndex) {
    for (; curIndex < candidates.size(); curIndex++) {
      int remaining = target - candidates[curIndex];
      if (remaining == 0) {
        curSeries.push_back(candidates[curIndex]);
        result.push_back(curSeries);
        return;
      } else if (remaining > 0) {
        vector<int> curSeriesCopy(curSeries);
        curSeriesCopy.push_back(candidates[curIndex]);
        combinationSumRecursion(candidates, remaining, result, curSeriesCopy,
                                curIndex);
      } else {
        return;
      }
