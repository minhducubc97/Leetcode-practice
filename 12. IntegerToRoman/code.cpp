...
class Solution {
 public:
  string intToRoman(int num) {
    unordered_map<int, string> hashMap({{1, "I"},
                                        {5, "V"},
                                        {10, "X"},
                                        {50, "L"},
                                        {100, "C"},
    ...
    int remaining = num;
    for (int i = 3; i >= 0; i--) {
      int div = remaining / pow(10, i);
      if (div != 0) {
        if (div == 4) {
          result += hashMap.find(pow(10, i))->second +
                    hashMap.find(5 * pow(10, i))->second;
        } else if (div == 9) {
          result += hashMap.find(pow(10, i))->second +
                    hashMap.find(pow(10, i + 1))->second;
        } else if (div < 4) {
          for (int j = 0; j < div; j++) {
            result += hashMap.find(pow(10, i))->second;
          }
        } else {
          result += hashMap.find(5 * pow(10, i))->second;
          for (int j = 0; j < div - 5; j++) {
            result += hashMap.find(pow(10, i))->second;
          }
        }
        remaining -= div * pow(10, i);
      }
    }
