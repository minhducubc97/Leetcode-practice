...
// value is out of the range of representable values, 231 − 1 or −231 is
// returned.
...
class Solution {
 public:
  int myAtoi(string str) {
    ...
    if ((int(str[idx]) == 43) || (int(str[idx]) == 45)) {
      sign = str[idx];
      idx++;
    }
    while ((int(str[idx]) >= 48) && (int(str[idx]) <= 57)) {
      if (result > INT_MAX / 10 && sign != '-') {
        return INT_MAX;
      } else if (result > INT_MAX / 10 && sign == '-') {
        return INT_MIN;
      }
      result = result * 10;
      digit = str[idx] - '0';
      if (INT_MAX - 1 - digit < result && sign != '-') {
        return INT_MAX;
      } else if (INT_MAX - digit < result && sign == '-') {
        return INT_MIN;
      }
      result += digit;
      idx++;
